...
import { PaymentAgreementList } from "@/components/payment/PaymentAgreementList/PaymentAgreementList";
...
import { listPaymentAgreements } from "@/endpoints/payment/list-agreement";
import {
...
export default async function PaymentPage({ searchParams }: PaymentPageProps) {
  ...
  const query = resolvePaymentAgreementQuery(await searchParams);

  const [agreementResult, portfolioResult] = await Promise.all([
    listPaymentAgreements(
      context.sessionId,
      context.forwarded,
      toPaymentAgreementEndpointQuery(query),
    ),
  ...
  const data = agreementResult.ok ? agreementResult.data : null;

  const filtered =
    Boolean(query.search) ||
    Boolean(query.portfolioId) ||
    Boolean(query.agreementType) ||
  ...
  return (
    <div className={styles.page}>
      <PaymentHeader
        query={query}
        portfolios={portfolioResult.ok ? portfolioResult.items : []}
        total={data?.pagination.totalItems}
      />

      {data ? (
        <PaymentAgreementList data={data} query={query} filtered={filtered} />
      ) : (
        <Alert variant="error" title="Payments unavailable">
